...
class MySeleniumSuite extends PHPUnit_Extensions_Selenium2TestCase {
    ...
    public function setUp(){
        ...
        $this -> filename = __DIR__ . "/reports/pt-sql-injection-result.html";
        $this -> fp = fopen($this -> filename, 'w');
        $data = 'PT origin environment'. $this -> templateHeader["sql-header"];
        fwrite($this->fp, $data);
    }
...
public function trackorder(){
  ...
  $injectEmail = $this -> configInjection["email-inject"];
  $fields = array(
  "email" =>  urlencode("tester_order@yahoo.com"),
  "orderid" =>  urlencode($inject),
  "TracOrderUnregistered" =>  urlencode("Track"),
  );
  $content = $this->loadCurlHeader($url ,$fields , "-");
  echo "\n\nValidate Trackorder Form";
...
public function feedback(){
 ...
 public function validateStatusCode($url,$content,$formName){
   ...
   $statusCompare1 = !strpos($httpStatus, "Not Found");

  if ($statusCompare && $statusCompare1 !== false ) {
